...
module ExtensionOpt : ExtFuncT = 
  ...
  struct
    ...
    struct
      let cm1 = cardinal - 1
      type t = Inf | Fini of int
      let modpos a b = match a mod b with
        | x when x >= 0 -> x
        | x -> b + x
    ...
    let print_alpha_i i =
      Poly.print (Conv.poly_of_cycl (IntExt.Fini i))
    ...
    let div a b =
      let bm = inv b in mult a bm

    let print a =
      match a.rep_cycl with
      | IntExt.Inf -> print_int 0
      | IntExt.Fini k -> print_string "a^" ; print_int k

    let random() =
      let i = Random.int cardinal in
      let c = 
        if i = cardinal - 1 then IntExt.Inf
        else IntExt.Fini i 
      in
    ...
    let incorporer a =
      let p = if a <>CorpsBase.zero then
          Poly.monome a 0 
        else
          Poly.nul in
      let c = Conv.cycl_of_poly p in
    ...
    let extraire a =
      let p = a.rep_poly in
      if Poly.degre p > 0 then raise ExtraireImpossible
      else Poly.coef_constant p
  end;;
...
module ExtensionNonOpt : ExtFuncT = 
  ...
  struct
    ...
    let print_table () = ()
    let mult a b =
      Poly.modulo (Poly.mult a b) poly_irr

    let inv a =
      let d, u, _ = Poly.euclide a poly_irr in
      if d <> Poly.un then failwith "poly non inversible"
      else Poly.modulo u poly_irr
    ...
    let pow a n =
      Poly.powmod a n poly_irr

    let incorporer a =
      if a <> CorpsBase.zero then
        Poly.monome a 0 
      else
        Poly.nul
