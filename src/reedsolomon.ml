...
struct
  ...
  let facteur_consecutifs = 
    let rec aux = function
      | 0 -> []
      | k -> k :: aux (k-1)
    in List.rev (aux (Taille.delta-1))
  ...
  let euclide a b = 
    ...
    in aux Poly.un Poly.nul Poly.nul Poly.un a b
  ...
  let chien_algo u =
    ...
    let rec aux = function
      | k when k>d -> []
      | a -> (Cf.pow beta a)::(aux (a+1)) in

    let rec somme = function
      |[] -> Cf.zero
      |a::s -> Cf.add a (somme s) in
  ...
  let decodage_brut y =
    Poly.list_of_poly (Poly.quotient y gene)
  ...
  let decodage y =
    ...
    if s = Poly.nul then
      ...
      let l = 
        (try (forney_algo erreur omega lambda) with _ -> print_string "NON";[]) in
      let rec poly_erreur = function
        | [] -> Poly.nul
        | (i,c) ::s  -> Poly.add (Poly.monome c i) (poly_erreur s) in
